...
for sr in situation_reports_links:
	sr_name = situation_reports_links[0].split("/")[-1].split("?")[0]
	available_sr_name.append(sr_name)

k = 0

for dl_link, name, i in zip(situation_reports_links, available_sr_name, range(len(available_sr_name), -1)):
	...
	if not os.path.exists(FILE_DIR):
		...
		print("Situation report - ", i+1, "downloaded as ", name)
		k += 1
